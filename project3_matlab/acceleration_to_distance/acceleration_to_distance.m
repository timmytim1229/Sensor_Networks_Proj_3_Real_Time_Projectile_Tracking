...
% Change this filename to the name of the file that  contains sensor data
% The file was previously in the same working directory as the Matlab
% working directory
filename = 'sensor_data_newlauncherquats1.txt';
...
% Get angles from gyro data in the same orientation as NED
temp = accel_y;
accel_y = accel_x*g;
accel_x = temp*g;
accel_z = - accel_z*g;
...
% Get data into angle instead of angles/sec
...
for i = 1:numVals
    ...
    %%% ACCELERATION to DISTANCE: METHOD 1 %%%
    % Rotation matrix from inertial to body frame for gyroscope
    ...
    % Rotation matrix from body to inertial frame for gyroscope
    ...
    %%% ACCELERATION to DISTANCE: METHOD 2 %%%
    % Compute quaternion rotation matrix for inertial to body frame
    ...
    % Invert quaternion matrix to get body to inertial frame
    iquat = inv(quat);   
     
    %%% Translate Sensor data to inertial (ground) frame using a rotation
    %%% matrix
    ...
    %%%a_body(i,:) = (R_ib*accel + [0; 0; g])';     % Use for METHOD 1
    a_body(i,:) = iquat * accel + [0; 0; g];        % Use for METHOD 2
...
plot(v)
[P_v,S_v] = polyfit([t;t]',[v(:,1) v(:,3)],1); % fit quadratic function to our velocity
Y_v = polyval(P_v,t);
...
plot(u)
[P_u,S_u] = polyfit([t;t]',[u(:,1) u(:,3)],1); % fit line function to our position
Y_u = polyval(P_u,t);
u(:,1) = u(:,1) - Y_u';
u(:,3) = u(:,3) - Y_u';
...
v_o_f = sqrt(-2*g*s*sin(angle)+(4/m)*(.5*k*s^2)-2*g*((2*ft_m/3)*.7)+2*ft_m*(.4+.2));
...
x = v_o*cos(angle)*t_p;
y_f = y_o + v_o_f*sin(angle).*t_p -.5*g*t_p.^2;
x_f = v_o_f*cos(angle)*t_p;
figure('Name','Projectile Motion','NumberTitle','on');

subplot(1,2,1)
plot(x,y)
title('Theoretical Pmax(rojectile Motion')
xlabel('x [m]');
ylabel('z [m]');
subplot(1,2,2)
plot(u(:,1), u(:,3));
title('Experimental Projectile Motion')

figure('Name','Projectile Motion','NumberTitle','on');

subplot(1,2,1)
plot(x_f,y_f)
title('Theoretical Projectile Motion (w/ Friction at Launch)')
xlabel('x [m]');
ylabel('z [m]');
subplot(1,2,2)
plot(-u(:,1)/1000, u(:,3)/1000);
title('Experimental Projectile Motion')
